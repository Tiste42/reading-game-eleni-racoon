...
test('original library recordings remain byte-identical and cannot be regenerated by TTS', async () => {
  const { readFileSync } = await import('node:fs');
  const { createHash } = await import('node:crypto');
  const manifest = JSON.parse(readFileSync('public/audio/phonemes/library-manifest.json', 'utf8'));
  assert.equal(Object.keys({ ...manifest.recordings, ...manifest.additionalLibraryRecordings }).length, 30);
  assert.deepEqual(manifest.pendingProvenance, []);
  for (const [file, provenance] of Object.entries({ ...manifest.recordings, ...manifest.additionalLibraryRecordings })) {
    const actual = createHash('sha256').update(readFileSync(`public/audio/phonemes/${file}`)).digest('hex');
    assert.equal(actual, (provenance as { sha256: string }).sha256, `Library asset was replaced: ${file}`);
  }
  const review = JSON.parse(readFileSync('tasks/phoneme-release-review.json', 'utf8'));
  assert.equal(review.clips.length, 30);
  for (const clip of review.clips) {
    assert.equal(clip.soundIdentityMatches, true, `Unresolved sound identity: ${clip.id}`);
    assert.equal(createHash('sha256').update(readFileSync(`public/audio/phonemes/${clip.id}.mp3`)).digest('hex'), clip.sha256, `Listening review is stale: ${clip.id}`);
  }
  const generator = readFileSync('scripts/generate-audio.ts', 'utf8');
  assert.ok(generator.includes('ElevenLabs generation is prohibited'));
  assert.ok(!generator.includes('PHONEME_TEXT'));
});
